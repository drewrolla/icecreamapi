...
def searchIceCream():
    ...
    if request.method == "POST":
        ...
        if form.validate():
            ...
            try:
                for icecream in icecreamShops:
                    shops = Icecream.query.filter_by(name=icecream['name']).first()
                    if not shops:
                        shops = Icecream(icecream['name'], icecream['rating'], icecream['address'], icecream['location'], icecream['state'], icecream['zipcode'], icecream['img_url'], icecream['website'])
                        shops.save()
            except KeyError:
